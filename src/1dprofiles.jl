...
Create a profile consisting of a specified function `f`.
The function must take in a single argument (time) and return
a single value.
...
```
"""
struct FunctionProfile <: Abstract1DProfile
    f :: Function
end
(p::FunctionProfile)(t) = p.f(t)

function show(io::IO, p::FunctionProfile)
    print(io, "Function profile")
end
